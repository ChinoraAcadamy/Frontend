	...
	{#await data.lazy.coursesData}
		<div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
			{#each Array(8) as _, i (i)}
				<div class="animate-pulse rounded-3xl border border-slate-100 bg-white p-5 shadow-sm">
					<div class="mb-4 flex items-start gap-4">
						<div class="h-20 w-20 shrink-0 rounded-2xl bg-slate-100"></div>
					...
					<div class="mt-auto mb-4">
						<div class="mb-2 h-4 w-1/3 rounded bg-slate-100"></div>
						<div class="flex items-center gap-3">
							<div class="h-1.5 flex-1 rounded-full bg-slate-100"></div>
							<div class="h-4 w-8 rounded bg-slate-100"></div>
						</div>
					</div>
