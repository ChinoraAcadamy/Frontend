	...
	try {
		const fetchLessonData = async () => {
			const res = await globalThis.fetch(
				`${API_URL}/courses/${params.id}/modules/${moduleId}/lessons/${params.lesson_id}/`,
				{ headers }
			);
			if (!res.ok) {
		...
		const fetchCourseDataForNextLesson = async () => {
			const res = await globalThis.fetch(`${API_URL}/courses/${params.id}/`, {
		...
		const lessonDataPromise = (async () => {
			...
			const rawLesson = await fetchWithCache(
				generateCacheKey('student_lesson_detail', user?.id, params.lesson_id),
				fetchLessonData
			);

			// Fetch module data to get accurate is_completed status
			const fetchModuleData = async () => {
				const res = await globalThis.fetch(`${API_URL}/courses/${params.id}/modules/${moduleId}/`, {
					headers
				});
				if (!res.ok) return null;
			...
			const moduleData = await fetchWithCache(
				generateCacheKey('student_module_detail', user?.id, moduleId),
				fetchModuleData
			);

			if (moduleData && moduleData.lessons) {
				const moduleLesson = moduleData.lessons.find(
					(l) => l.id.toString() === params.lesson_id.toString()
				);
		...
		const nextLessonPromise = (async () => {
			await Promise.resolve(); // Defer to let SvelteKit serialize & attach error listeners
			const courseData = await fetchWithCache(
				generateCacheKey('student_course_base', user?.id, params.id),
				fetchCourseDataForNextLesson
			);
...
export const actions = {
	uploadAssignment: async ({ request, cookies, fetch }) => {
		...
		if (!assignmentId) {
			return {
				success: false,
				error: m.error_occurred ? m.error_occurred() : 'Topshiriq ID topilmadi.'
			};
		}
