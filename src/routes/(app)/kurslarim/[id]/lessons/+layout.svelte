...
<div class="mx-auto max-w-7xl text-slate-800 sm:px-6 lg:px-8">
	<div class="mb-4 h-2"></div>

	<!-- Mobile Sidebar Toggle -->
	<div class="mx-2 mb-4 block lg:hidden">
		<button
			onclick={() => (showMobileSidebar = !showMobileSidebar)}
			class="flex w-full items-center justify-between rounded-xl border border-main bg-card px-2 py-1.5 text-sm font-bold text-main shadow-sm transition-colors hover:bg-muted/5"
		>
	...
	<div class="grid grid-cols-1 items-start gap-6 lg:grid-cols-[320px_1fr] xl:grid-cols-[340px_1fr]">
		...
		>
			{#await data.lazy.moduleData}
				<div
					class="flex h-64 flex-col items-center justify-center rounded-3xl border border-main bg-card shadow-sm"
				>
					<div
			...
			{:then module}
				{#if module}
					<div
						class="flex max-h-[calc(100vh-120px)] flex-col overflow-hidden rounded-3xl border border-main bg-card shadow-sm"
					>
						<!-- Sidebar Header -->
						<div class="border-b border-main bg-muted/5 p-5">
								...
								<div class="mt-3 flex flex-col gap-1.5">
									<div class="flex items-center justify-between text-[11px] font-bold text-muted">
										<span class="tracking-wider uppercase"
											>{m.progress ? m.progress() : 'Progress'}</span
										>
							...
							<div class="flex flex-col gap-1.5">
								{#each module.lessons as l (l.id)}
									...
									{#if l.can_access === false && !isAdmin}
										<div
											class="flex cursor-not-allowed items-center justify-between gap-3 rounded-2xl border border-main/50 bg-muted/5 p-3 opacity-60"
										>
									...
									{:else}
										<a
											...
											class="group flex items-center justify-between gap-3 rounded-2xl p-3 transition-colors {l.id.toString() ===
											$page.params.lesson_id
										...
										>
											<div class="flex min-w-0 items-start gap-3">
												...
												<div class="flex min-w-0 flex-col">
													<span
														class="line-clamp-2 text-sm font-bold {l.id.toString() ===
														$page.params.lesson_id
															? 'text-primary'
