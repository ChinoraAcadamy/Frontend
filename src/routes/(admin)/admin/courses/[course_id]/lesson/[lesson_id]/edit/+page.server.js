...
export const load = async ({ fetch, cookies, params }) => {
	...
	try {
		const response = await fetch(
			`${API_URL}/courses/${params.course_id}/modules/${parentModulePk}/lessons/${params.lesson_id}/`,
			{ headers }
		);
...
export const actions = {
	updateLesson: async ({ fetch, cookies, request, params }) => {
		...
		try {
			const response = await fetch(
				`${API_URL}/courses/${params.course_id}/modules/${moduleId}/lessons/${params.lesson_id}/`,
				{
					method: 'PATCH',
					headers,
					body: formData
