...
export const load = async (event) => {
	...
	return {
		...
		lazy: {
			...
			ranking: fetchWithCache(generateCacheKey('admin_ranking', userId), () =>
				getRanking(event).then((d) => d?.results || [])
			),
			newStudents: fetchWithCache(generateCacheKey('admin_new_students', userId), fetchNewStudents),
			activityLogs: fetchWithCache(
				generateCacheKey('admin_activity_logs_short', userId),
				getActivityLogs
			)
		}
	};
