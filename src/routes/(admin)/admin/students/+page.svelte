...
{#snippet toastNotification()}
	{#if toast}
		...
		>
			<CheckCircle2 size={18} class={toast.type === 'success' ? 'text-primary' : 'text-primary'} />
			{toast.message}
		</div>
	{/if}
{/snippet}
...
<div class="min-h-screen bg-main p-4 font-sans sm:p-6 lg:p-8">
	<div class="mx-auto max-w-7xl">
		...
		<div class="mb-8 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
			<div>
				<h1 class="text-2xl font-black tracking-tight text-main">
				...
				{:else if resolvedData && resolvedData.totalCount !== undefined}
					<p class="mt-1 text-sm font-medium text-muted">
						...
						<span class="font-bold text-primary">{resolvedData.totalCount}</span>
						{m.admin_students_count_suffix ? m.admin_students_count_suffix() : 'ta student'}
					</p>
				{/if}
			</div>
			...
			<button
				...
				class="hover:bg-primary-hover flex h-11 items-center gap-2 rounded-xl bg-primary px-6 text-sm font-bold text-white shadow-lg shadow-primary/20 transition-all focus:ring-4 focus:ring-primary/20 active:scale-95"
			>
		...
		<div class="mb-6 flex flex-col gap-3 md:flex-row">
			<div class="relative flex-1 md:max-w-md">
				...
				<input
					type="text"
					placeholder={m.admin_students_search_placeholder
						? m.admin_students_search_placeholder()
						: "Ism yoki username bo'yicha qidirish..."}
			...
			>
				<option value="all"
					>{m.admin_students_filter_all
						? m.admin_students_filter_all()
						: 'Barcha statuslar'}</option
				>
				<option value="true"
					>{m.admin_students_filter_active ? m.admin_students_filter_active() : 'Faol'}</option
				>
				<option value="false"
					>{m.admin_students_filter_inactive
						? m.admin_students_filter_inactive()
						: 'Nofaol'}</option
				>
			</select>
		...
		{:else if resolvedData?.error}
			<div
				class="rounded-xl border border-red-200 bg-red-50 p-6 text-center text-sm font-bold text-red-600"
			>
				{m.admin_students_error_loading
					? m.admin_students_error_loading({ error: resolvedData.error })
					: `Ma'lumotlarni yuklashda xatolik yuz berdi. ${resolvedData.error}`}
			</div>
		{:else}
			<div class="overflow-hidden rounded-2xl border border-main bg-card shadow-sm">
				<div class="overflow-x-auto">
					<table class="w-full min-w-[900px] text-left text-sm">
						<thead class="border-b border-main bg-muted/5">
							<tr>
								<th class="admin-table"
									>{m.admin_students_col_name ? m.admin_students_col_name() : 'Ism Familiya'}</th
								>
								<th class="admin-table"
									>{m.admin_students_col_username
										? m.admin_students_col_username()
										: 'Username'}</th
								>
								<th class="admin-table"
								...
								>
								<th class="admin-table text-center"
									>{m.admin_students_col_courses ? m.admin_students_col_courses() : 'Kurslar'}</th
								>
								<th class="admin-table"
									>{m.admin_students_col_status ? m.admin_students_col_status() : 'Status'}</th
								>
								<th class="admin-table w-32 text-right font-bold"
								...
								<tr class="group transition-colors hover:bg-muted/5">
									<td class="px-6 py-4">
										...
										>
											...
											<span class="font-bold text-main transition-colors group-hover:text-primary"
												>{getFullName(student)}</span
											>
										</a>
									</td>
									<td class="px-6 py-4">
				...
				{#if totalPages > 1}
					<div class="flex items-center justify-between border-t border-main bg-card px-6 py-4">
						<button
							...
							class="flex h-9 items-center gap-1 rounded-xl bg-muted/5 px-4 text-xs font-bold tracking-widest text-muted uppercase transition-colors hover:bg-muted/10 disabled:opacity-40 disabled:hover:bg-muted/5"
						>
							<ChevronLeft size={16} />
...
<style>
	...
	.admin-table {
		@apply px-6 py-4 text-xs font-bold tracking-widest uppercase;
	}
</style>
