<script>
	...
	let formatUser = (user, userNameStr, attemptStr) => {
		...
		return (
			attemptStr || (m.admin_student_unknown_date ? m.admin_student_unknown_date() : "Noma'lum")
		);
	};
</script>
		...
		<div class="mb-8 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
			<div>
				<h1 class="flex items-center gap-2 text-2xl font-semibold text-main">
					<KeyRound class="text-primary" />
					{m.admin_logs_login_title ? m.admin_logs_login_title() : 'Tizimga kirish tarixi'}
				</h1>
				{#await data.lazy.loginData then resolvedData}
					<p class="mt-1 text-sm text-muted">
						...
						{m.admin_logs_login_count_suffix ? m.admin_logs_login_count_suffix() : 'ta urinish'}
					</p>
				{/await}
			</div>
		</div>
		...
		<form method="GET" class="mb-6 flex flex-col gap-3 md:flex-row">
			<div class="relative flex-1 md:max-w-md">
				...
				<input
					type="text"
					name="search"
					placeholder={m.admin_logs_login_search
						? m.admin_logs_login_search()
						: 'Ism, username yoki IP manzilni izlash...'}
						...
						<thead class="border-b border-main bg-muted/5">
							<tr>
								<th class="admin-table"
									>{m.admin_logs_login_col_attempt
										? m.admin_logs_login_col_attempt()
										: 'Urinish (Username)'}</th
								>
								<th class="admin-table hidden sm:table-cell"
									>{m.admin_logs_login_col_status
										? m.admin_logs_login_col_status()
										: 'Holat (Status)'}</th
								>
								<th class="admin-table hidden md:table-cell"
									>{m.admin_logs_login_col_device
										? m.admin_logs_login_col_device()
										: 'Qurilma (Device)'}</th
								>
								<th class="admin-table hidden sm:table-cell"
							...
							{#each logs as log (log.id)}
								...
								<tr class="group transition-colors hover:bg-muted/5">
									...
									<td class="hidden px-6 py-4 sm:table-cell">
										<span
											class="inline-flex items-center gap-1.5 rounded-md px-2.5 py-1 text-xs font-semibold ring-1 ring-inset {config.classes}"
										>
											<config.Icon size={14} class={config.iconClass} />
											{config.label}
										</span>
									</td>
									<td class="hidden px-6 py-4 text-muted md:table-cell">
										<div class="flex flex-col gap-0.5">
											<span class="flex items-center gap-1 text-xs font-medium text-main">
												<MonitorSmartphone size={14} />
												{log.device_info
													? log.device_info.split(',')[0]
													: m.admin_logs_login_unknown_device
														? m.admin_logs_login_unknown_device()
														: "Noma'lum Qurilma"}
											</span>
											<span
												class="max-w-[200px] truncate text-xs text-muted/60"
									...
									<td class="hidden px-6 py-4 font-mono text-xs text-muted sm:table-cell">
										{log.ip_address || '—'}
									</td>
