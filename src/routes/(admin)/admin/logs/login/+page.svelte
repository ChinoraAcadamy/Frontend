		...
		<div class="mb-8 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
			<div>
				<h1 class="flex items-center gap-2 text-2xl font-semibold text-gray-900">
					<KeyRound class="text-indigo-600" /> Tizimga kirish tarixi
				</h1>
				{#await data.lazy.loginData then resolvedData}
					<p class="mt-1 text-sm text-gray-500">
						Jami <span class="font-medium text-gray-900">{resolvedData.count}</span> ta urinish
					</p>
				{/await}
			</div>
		</div>
		...
		<form method="GET" class="mb-6 flex flex-col gap-3 md:flex-row">
			<div class="relative flex-1 md:max-w-md">
				...
				<input
					type="text"
					name="search"
					placeholder="Ism, username yoki IP manzilni izlash..."
				...
				<div class="overflow-x-auto">
					<table class="w-full min-w-[1000px] text-left text-sm">
						<thead class="border-b border-gray-200 bg-gray-50/80">
							<tr>
								<th class="admin-table">Urinish (Username)</th>
								<th class="admin-table hidden sm:table-cell">Holat (Status)</th>
								<th class="admin-table hidden md:table-cell">Qurilma (Device)</th>
									...
									<td class="px-6 py-4 text-gray-500 hidden md:table-cell">
										<div class="flex flex-col gap-0.5">
											<span class="flex items-center gap-1 text-xs font-medium text-gray-700">
												...
												{log.device_info ? log.device_info.split(',')[0] : 'Noma\'lum Qurilma'}
											</span>
				...
				{#if totalPages > 1}
					<div class="flex items-center justify-between border-t border-gray-200 bg-white px-6 py-4">
						...
						<div class="flex gap-1">
							<span class="text-sm font-medium text-gray-600">Sahifa {currentPage} / {totalPages}</span>
						</div>
						<a
