...
<Modal {isOpen} {onClose} maxWidth="450px" noPadding={true}>
	<div class="custom-modal-override flex flex-col">
		<div
			class="flex items-center justify-between rounded-t-[24px] border-b border-main bg-muted/5 p-6"
		>
			<div>
		...
		>
			...
			<div class="flex gap-3 border-t border-main pt-4">
				...
				<button
					...
					class="hover:bg-primary-hover flex-1 rounded-xl bg-primary py-3.5 font-bold text-white shadow-lg shadow-primary/20 transition-all active:scale-95 disabled:opacity-60"
				>
					{isSubmitting
						? m.profile_saving
							? m.profile_saving()
