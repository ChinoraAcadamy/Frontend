	...
	{#if isA11yOpen}
		...
		>
			<div class="panel-head">
				<div class="panel-head-left">
					...
					<div>
						...
						<p class="panel-sub">
							{activeCount > 0
								? (m.a11y_active_count?.({ count: activeCount }) ?? `${activeCount} ta yoqilgan`)
								: (m.a11y_default_settings?.() ?? 'Standart sozlamalar')}
						</p>
					</div>
				</div>
			...
			<div class="panel-body">
				<!-- SECTION: Vision -->
				<div class="p-section">
					<p class="p-section-label">
						<Sun size={12} strokeWidth={2.5} />
						{m.a11y_vision?.() ?? "Ko'rish"}
					</p>
					...
					<div class="toggle-grid">
						<button
							class="tgl-btn {v.isGrayscale ? 'tgl-btn--on' : ''}"
							onclick={() => (v.isGrayscale = !v.isGrayscale)}
						>
							<Palette size={15} strokeWidth={2} />
							<span>{m.admin_a11y_grayscale?.() ?? 'Oq-qora'}</span>
						</button>
						<button
							class="tgl-btn {v.isHighContrast ? 'tgl-btn--on' : ''}"
							onclick={() => (v.isHighContrast = !v.isHighContrast)}
						>
				...
				<div class="p-section">
					<p class="p-section-label">
						...
						{m.a11y_color_vision?.() ?? "Rang ko'rish"}
					</p>
					<div class="cb-grid">
						{#each [{ id: 'none', label: m.a11y_normal?.() ?? 'Normal' }, { id: 'protanopia', label: m.a11y_protanopia?.() ?? 'Protan' }, { id: 'deuteranopia', label: m.a11y_deuteranopia?.() ?? 'Deuteran' }, { id: 'tritanopia', label: m.a11y_tritanopia?.() ?? 'Tritan' }, { id: 'achromatopsia', label: m.a11y_achromatopsia?.() ?? 'Akromat' }] as cb (cb.id)}
							<button
								class="cb-btn {v.colorBlindMode === cb.id ? 'cb-btn--on' : ''}"
								onclick={() => (v.colorBlindMode = cb.id)}>{cb.label}</button
							>
						{/each}
					</div>
				</div>
				...
				<div class="p-section">
					...
					<div class="p-row p-row--stepper">
						...
						<div class="stepper">
							<button
								class="step-btn"
								onclick={() => (v.zoomLevel = clamp(v.zoomLevel - 0.1, 0.8, 1.6))}
					...
					<div class="p-row p-row--stepper">
						...
						<div class="stepper">
							<button
								class="step-btn"
								onclick={() => (v.lineHeight = +clamp(v.lineHeight - 0.1, 1.2, 2.2).toFixed(1))}
								><Minus size={14} /></button
							>
					...
					<div class="p-row p-row--stepper">
						...
						<div class="stepper">
							<button
								class="step-btn"
								onclick={() =>
									(v.letterSpacing = +clamp(v.letterSpacing - 0.02, 0, 0.2).toFixed(2))}
								><Minus size={14} /></button
							>
					...
					<div class="p-row">
						...
						<div class="align-btns">
							{#each [{ v: 'left', Icon: AlignLeft }, { v: 'center', Icon: AlignCenter }, { v: 'justify', Icon: AlignJustify }] as a (a.v)}
								<button
									class="align-btn {v.textAlign === a.v ? 'align-btn--on' : ''}"
									onclick={() => (v.textAlign = a.v)}
								>
					...
					<button
						class="full-toggle {v.isDyslexiaFont ? 'full-toggle--on' : ''}"
						onclick={() => (v.isDyslexiaFont = !v.isDyslexiaFont)}
					>
				...
				{#if hasMouse}
					<div class="p-section">
						<p class="p-section-label">
							...
							{m.a11y_navigation?.() ?? 'Navigatsiya'}
						</p>

						<div class="p-row">
							<span class="p-row-label">{m.a11y_cursor_size?.() ?? 'Kursor'}</span>
							<div class="seg-btns">
								{#each [{ v: 'default', label: 'Standart' }, { v: 'large', label: 'Katta' }, { v: 'xl', label: 'XL' }] as c (c.v)}
									<button
										class="seg-btn {v.cursorSize === c.v ? 'seg-btn--on' : ''}"
										onclick={() => (v.cursorSize = c.v)}>{c.label}</button
									>
								{/each}
							</div>
						</div>
				...
				<div class="p-section p-section--last">
					<p class="p-section-label">
						...
						{m.a11y_reading?.() ?? 'Qulayliklar'}
					</p>

					<button
						class="full-toggle {v.isLinkHighlight ? 'full-toggle--on' : ''}"
						onclick={() => (v.isLinkHighlight = !v.isLinkHighlight)}
					>
						<div class="full-toggle-left">
					...
					<button
						class="full-toggle {v.isFocusHighlight ? 'full-toggle--on' : ''}"
						onclick={() => (v.isFocusHighlight = !v.isFocusHighlight)}
					>
						<div class="full-toggle-left">
					...
					<button
						class="full-toggle {v.isHideImages ? 'full-toggle--on' : ''}"
						onclick={() => (v.isHideImages = !v.isHideImages)}
					>
						<div class="full-toggle-left">
