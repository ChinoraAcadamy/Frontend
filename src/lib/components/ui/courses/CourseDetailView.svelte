...
<div class="relative mx-auto min-h-[calc(100vh-80px)] max-w-5xl bg-background p-4 font-sans md:p-8">
	{#if isStudentBlocked}
		<div
			class="mb-6 flex items-start gap-4 rounded-2xl border border-red-500/20 bg-red-500/10 p-4 shadow-sm sm:p-6"
		>
			<div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-red-500/20">
				<Lock size={20} class="text-red-500" />
			</div>
			<div>
				<h3 class="mb-1 text-lg font-bold text-red-500">
					{m.course_locked()}
				</h3>
				<p class="text-sm leading-relaxed font-medium text-red-500/80">
	...
	>
		<div class="mb-6 flex flex-col justify-between gap-4 md:flex-row md:items-end">
			<div>
				<h1 class="text-2xl font-bold text-foreground sm:text-3xl">{course.title}</h1>
			</div>
		...
		<div class="mb-8 grid grid-cols-1 gap-6 lg:grid-cols-3">
			<div
				class="flex flex-col items-start gap-5 rounded-3xl border border-border bg-surface p-5 shadow-sm sm:flex-row sm:items-center sm:gap-6 sm:p-6 lg:col-span-2"
			>
				<div
				...
				<div class="w-full flex-1">
					<h2 class="mb-2 text-xl font-bold text-foreground">{course.title}</h2>
					<p class="mb-4 line-clamp-2 text-sm leading-relaxed text-muted">
						{course.description}
					</p>

					<div class="flex items-center gap-4">
						<div class="h-2 flex-1 overflow-hidden rounded-full bg-muted/10">
							<div
								class="h-full rounded-full bg-primary transition-all duration-700 ease-out"
								style="width: {course.progress || 0}%"
							></div>
						</div>
						<span class="text-sm font-semibold text-foreground">{course.progress || 0}%</span>
					</div>
				</div>
			</div>

			<div
				class="flex flex-col justify-center rounded-3xl border border-border bg-surface p-6 shadow-sm"
			>
		...
		<div class="space-y-4">
			{#each modules as mod (mod.title)}
				<ModuleAccordion title={mod.title}>
					...
					{:else}
						<p class="py-2 text-sm text-muted italic">
			...
			{:else}
				<div
					class="flex flex-col items-center justify-center p-10 bg-surface rounded-3xl border border-border text-center"
				>
					<p class="text-muted font-medium">{m.course_no_modules()}</p>
				</div>
			{/each}
		</div>
