...
<div class="mx-auto min-h-[calc(100vh-80px)] max-w-5xl bg-[#f8fafc] p-4 font-sans md:p-8 relative">
    
    {#if isStudentBlocked}
        <div class="mb-6 rounded-2xl bg-red-50 border border-red-200 p-4 sm:p-6 flex items-start gap-4 shadow-sm">
            <div class="w-10 h-10 rounded-full bg-red-100 flex items-center justify-center shrink-0">
                <Lock size={20} class="text-red-600" />
            </div>
            <div>
                <h3 class="text-red-800 font-bold text-lg mb-1">Kurs bloklangan</h3>
                <p class="text-red-600 font-medium text-sm leading-relaxed">
                    Ushbu kursga kirish huquqingiz cheklangan. Darslarni ko'rish va davom ettirish uchun ma'muriyat bilan bog'laning yoki to'lovni amalga oshiring.
                </p>
            </div>
        </div>
    {/if}

	<div class="{isStudentBlocked ? 'opacity-50 pointer-events-none grayscale-[30%]' : ''} transition-all duration-300">
		<!-- Header -->
					...
					<div class="flex flex-col gap-2">
						{#each mod.lessons as lesson (lesson.id)}
							<LessonsRow
								...
								status={isStudentBlocked ? 'locked' : 'available'}
								href={getLessonHref && !isStudentBlocked ? getLessonHref(lesson.id, mod.id) : undefined}
							/>
						{/each}
					</div>
				{:else}
