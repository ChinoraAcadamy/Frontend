...
<!-- Main Content: Video & Info & Assignments -->
<div class="flex min-w-0 flex-col gap-6">
	<!-- Video Player -->
	<div
		class="relative aspect-video w-full overflow-hidden rounded-3xl border border-main bg-main shadow-sm"
	>
		{#if !player}
			<div
				class="absolute inset-0 z-20 flex items-center justify-center bg-main/50 backdrop-blur-sm"
				transition:fade
			>
				<div
	...
	{#if hasAssignments}
		<div
			class="mt-4 flex flex-col overflow-hidden rounded-3xl border border-main bg-card shadow-sm"
		>
				...
				<div in:fly={{ x: 20, duration: 400 }} class="flex flex-col">
					...
					<div class="bg-card p-5 pb-2 sm:p-6">
						<div class="mb-4 flex items-center justify-between">
							<span
								class="rounded border border-main bg-muted/5 px-2 py-1 text-[11px] font-bold tracking-wider text-muted uppercase"
							>
						...
						{#if assignment.type === 'file'}
							...
							>
								...
								{#if selectedFile}
									...
									<span class="line-clamp-1 text-center text-sm font-bold text-main"
					...
					{#if assignment.submissions && assignment.submissions.length > 0}
						<div class="flex flex-col border-t border-main bg-muted/5 p-5 sm:p-6">
							<h3 class="mb-4 text-sm font-bold text-main">
								{m.assignment_prev_attempts()}
							</h3>
							<div class="flex flex-col gap-3">
								{#each assignment.submissions as sub (sub.id)}
									<div
										class="flex flex-col gap-3 rounded-md border border-main bg-card p-4"
										transition:slide
									>
										<div class="flex items-center justify-between">
											<div class="flex items-center gap-2 text-[11px] font-medium text-muted">
												<span
													>{new Date(sub.submitted_at || Date.now()).toLocaleDateString('uz')}</span
												>
												<span class="h-1 w-1 rounded-full bg-muted/30"></span>
												<span
													>{new Date(sub.submitted_at || Date.now()).toLocaleTimeString('uz')}</span
												>
											</div>
											{#if sub.status === 'Graded' || sub.status === 'graded'}
												<span
													class="rounded border border-main bg-muted/10 px-2 py-1 text-[10px] font-bold tracking-wider text-muted uppercase"
											...
											{:else}
												...
												>
													<span class="text-[10px] font-bold tracking-wider text-sky-600 uppercase"
										...
										{#if sub.feedback}
											<div class="mt-1 border-l-2 border-primary bg-muted/5 p-3">
												...
												>
												<p class="text-[13px] font-medium text-main italic">
													"{sub.feedback}"
												</p>
											</div>
										{/if}
...
{#if securityLocked}
	<div
		class="fixed inset-0 z-9999 flex flex-col items-center justify-center bg-main px-6 text-center text-main backdrop-blur-xl"
		transition:fade
	>
		<div
		...
		<div class="mt-10 flex flex-col items-center gap-4">
			<button
				...
				class="rounded-2xl bg-card px-8 py-3.5 text-sm font-black tracking-widest text-main uppercase transition-all hover:scale-105 active:scale-95"
			>
