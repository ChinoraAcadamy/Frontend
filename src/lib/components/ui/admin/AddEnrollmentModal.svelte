	...
	>
		...
		>
			<div class="flex items-center justify-between border-b border-slate-100 bg-slate-50/50 p-6">
				<div>
					<h2 class="text-xl font-bold text-slate-800">{m.admin_student_add_course ? m.admin_student_add_course() : "Kurs qo'shish"}</h2>
				...
				use:enhance={() => {
					...
					return async ({ result, update }) => {
						...
						if (result.type === 'success') {
							showToast(m.enroll_success ? m.enroll_success() : "Kurs muvaffaqiyatli qo'shildi!", 'success');
							selectedCourseIds = [];
							searchQuery = '';
							onClose();
							...
							showToast(
								typeof err === 'string' ? err : (m.enroll_error ? m.enroll_error() : 'Kurs qo\'shishda xatolik yuz berdi!'),
								'error'
							);
						}
						await update();
			...
			>
				<div class="mb-4">
					<div class="relative flex items-center">
						...
						<input
							...
							placeholder={m.courses_search_placeholder ? m.courses_search_placeholder() : "Kurslarni qidirish..."}
									...
									<div class="flex items-center gap-3">
										...
										<div>
											...
											<p class="text-xs {isEnrolled ? 'text-emerald-500 font-semibold' : 'text-slate-500'}">
												...
												{:else}
													{course.price ? course.price.toLocaleString() + ' ' + (m.price_label ? m.price_label() : 'UZS') : (m.course_free ? m.course_free() : 'Bepul')}
												{/if}
											</p>
										</div>
									</div>
				...
				<div class="mt-6 flex gap-3 border-t border-slate-100 pt-5">
					...
					>
						{isSubmitting ? (m.enrolling ? m.enrolling() : 'Biriktirilmoqda...') : (m.enroll_btn ? m.enroll_btn() : 'Biriktirish') + ` (${selectedCourseIds.length})`}
					</button>
				</div>
			</form>
		</div>
