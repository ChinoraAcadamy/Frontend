...
<div class="flex flex-col gap-1.5 relative">
    ...
    <button
        ...
        class="w-full flex items-center justify-between px-4 py-3 bg-white border rounded-lg transition-colors outline-none
               {isDropdownOpen ? 'border-[#fa2e69]' : 'border-slate-200 hover:border-slate-300'}"
    >
        {#if selectedModule}
    ...
    {#if isDropdownOpen}
        <div 
            class="absolute top-[calc(100%+4px)] left-0 right-0 bg-white rounded-lg border border-slate-200 shadow-xl z-50 overflow-hidden" 
            transition:slide={{ duration: 150 }}
        >
            <div class="flex items-center gap-2 px-3 py-2 border-b border-slate-100 bg-slate-50/50">
            ...
            <div class="max-h-[200px] overflow-y-auto divide-y divide-slate-50">
                {#each filteredModules as module (module.id)}
                    ...
                    >
                        <span class="text-xs font-semibold text-slate-600">{module.title}</span>
