...
<section
	bind:this={section}
	class="header-background flex min-h-screen flex-col items-center justify-between overflow-hidden px-8 md:flex-row md:px-16 lg:px-24"
>
	<div class="z-10 max-w-2xl flex-1 text-left">
		<h1
			class="hero-title mt-32 mb-6 text-3xl leading-snug font-bold text-foreground sm:mt-20 md:mt-0 md:text-4xl lg:text-5xl"
		...
		<p class="hero-subtitle text-muted-foreground max-w-lg text-base leading-relaxed md:text-lg">
			{@html m.hero_subtitle({
				income: `<span class="font-semibold text-primary">${m.hero_income_highlight()}</span>`
			})}
		</p>
		...
		>
			<span
				class="relative flex h-6 w-6 items-center justify-center rounded-full bg-background text-primary transition-transform duration-300 group-hover:rotate-45"
			>
